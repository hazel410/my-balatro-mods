...
function UIBox:calculate_xywh(node, _T, recalculate, _scale)
    ...
    if node.UIT == G.UIT.B or node.UIT == G.UIT.T or node.UIT == G.UIT.O then
        ...
        for i = 1, 2 do
            if i == 1 or (i == 2 and ((node.config.maxw and _ct.w > node.config.maxw) or (node.config.maxh and _ct.h > node.config.maxh))) then 
                ...
                if i == 2 then
                    ...
                    if node.config.no_overflow and string.find(node.config.no_overflow, node.config.maxw and "h" or "v") then fac = _scale or 1 end
                end
                _nt.x, _nt.y, _nt.w, _nt.h = 
                    _T.x,
                    _T.y,
        ...
        if node.config and node.config.no_overflow then
            if string.find(node.config.no_overflow, "h") then
            ...
            if string.find(node.config.no_overflow, "v") then
    ...
    if next(self.children) == nil or self.config.no_fill then
        ...
        for k, w in pairs(self.children) do
            ...
            if w.config and w.config.no_overflow then
                if string.find(w.config.no_overflow, "h") then
                    if w.config.w then
                        w.T.w = w.config.w
                ...
                if string.find(w.config.no_overflow, "v") then
                    if w.config.h then
                        w.T.h = w.config.h
...
function UIElement:draw_self()
    ...
    self.ARGS.cached_draw = self.ARGS.cached_draw or function(self, shader, send, button_active, parallax_dist, button_being_pressed)
    if self.UIT ~= G.UIT.T then
        self:set_element_shader(shader, send, false)
    end
            ...
            if (self.config.button_UIE and button_active) or (not self.config.button_UIE and self.config.shadow and G.SETTINGS.GRAPHICS.shadows == 'On') then 
                ...
                if (self.config.shadow or (self.config.button_UIE and button_active)) and G.SETTINGS.GRAPHICS.shadows == 'On' then
                    ...
                    for _, v in ipairs(text_shaders) do
                        if v then self:set_text_shader(v.shader, v.send, true) end
                        love.graphics.draw(
    ...
    if self.UIT ~= G.UIT.T then
        self:set_element_shader()
    end
    end
    
    local element_shaders = SMODS.resolve_ui_shaders(self, self.states.visible and self.UIT ~= G.UIT.T and self.config.shader, nil)
    for _, v in ipairs(element_shaders) do
        if v then
            self.ARGS.cached_draw(self, v.shader, v.send, button_active, parallax_dist, button_being_pressed)
        else
            self.ARGS.cached_draw(self, nil, nil, button_active, parallax_dist, button_being_pressed)
        end
    end
