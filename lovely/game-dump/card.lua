...
SMODS.enh_cache:write(self, nil)
    ...
    if not initial and G.GAME.blind then G.GAME.blind:debuff_card(self) end
...
function Card:get_bonus_x_score()
    if self.debuff then return 0 end
    local ret = SMODS.multiplicative_stacking(1, self.ability.perma_x_score or 0)
...
function Card:get_bonus_h_x_score()
    if self.debuff then return 0 end
    local ret = SMODS.multiplicative_stacking(1, self.ability.perma_h_x_score or 0)
    -- TARGET: get_bonus_h_x_score
    return ret
end
function Card:get_bonus_blind_size()
    if self.debuff then return 0 end
    local ret = (self.ability.perma_blind_size or 0)
    -- TARGET: get_bonus_blind_size
    return ret
end

function Card:get_bonus_x_blind_size()
    if self.debuff then return 1 end
    local ret = SMODS.multiplicative_stacking(1, self.ability.perma_x_blind_size or 0)
    -- TARGET: get_bonus_x_blind_size
    return ret
end

function Card:get_bonus_h_blind_size()
    if self.debuff then return 0 end
    local ret = (self.ability.perma_h_blind_size or 0)
    -- TARGET: get_bonus_h_blind_size
    return ret
end

function Card:get_bonus_h_x_blind_size()
    if self.debuff then return 1 end
    local ret = SMODS.multiplicative_stacking(1, self.ability.perma_h_x_blind_size or 0)
    -- TARGET: get_bonus_h_x_blind_size
    return ret
end
...
function Card:check_use()
    if self.area == G.pack_cards and G.pack_cards and booster_obj and SMODS.card_select_area(self, booster_obj) and self:selectable_from_pack(booster_obj) then return end
    if self.ability.name == 'Ankh' then 
    ...
    if self.ability.set == "Booster" then
        ...
        G.E_MANAGER:add_event(Event({trigger = 'after', delay = 0.4, func = function()
            ...
            G.E_MANAGER:add_event(Event({trigger = 'after', delay = 1.3*math.sqrt(G.SETTINGS.GAMESPEED), blockable = false, blocking = false, func = function()
                ...
                for i = 1, _size do
                    local card = nil
                    local flags = SMODS.calculate_context({create_booster_card = true, booster = self, index = i})
                    if flags.booster_create_flags then
