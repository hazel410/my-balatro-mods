...
function level_up_hand(card, hand, instant, amount, statustext)
    ...
    SMODS.upgrade_poker_hands({
        ...
        func = function(base, hand, parameter, level_up)
                return base + G.GAME.hands[hand]['l_' .. parameter] * level_up
        end,
        level_up = amount,
        from = card,
        instant = instant,
        ...
        G.E_MANAGER:add_event(Event({
            ...
            func = function()
                ...
                elseif string.find(config.name, 'joker') then
                    ...
                    table.insert(left_text, {n=G.UIT.O, config={object = DynaText({string = loc_opts.text or localize{type = 'name_text', set = loc_opts.set or config.card.config.center.set, key = loc_opts.key or config.card.config.center.key}, colours = {loc_opts.text_colour or G.C.FILTER}, shadow = true, pop_in = 0, scale = (loc_opts.scale or 0.6)*scale, silent = true})}})
...
function check_for_unlock(args)
    ...
    while i <= #G.P_LOCKED do
        ...
        if not custom_check and not card.unlocked and card.unlock_condition and card.unlock_condition.type == args.type then
            ...
            if args.type == 'min_hand_size' and G.hand and G.hand.config.card_limits.total_slots <= card.unlock_condition.extra then
                ret = true
                unlock_card(card)
            end
...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    local front = SMODS.set_create_card_front or ((_type=='Base' or _type == 'Enhanced') and pseudorandom_element(G.P_CARDS, pseudoseed('front'..(key_append or '')..G.GAME.round_resets.ante))) or nil
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end, card)
    ...
    elseif _c.generate_ui and type(_c.generate_ui) == 'function' then
        ...
        -- TODO: refactor the multiple generate_ui cases to work off a base implementation
   ...
   --Fill all remaining info if this is the main desc
    ...
    if not hide_desc and card and card.ability and (card.ability.extra_slots_used or 0) ~= 0 then
    ...
    if not hide_desc and card and card.ability and (card.ability.card_limit or 0) ~= 0 then
